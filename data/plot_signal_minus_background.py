...
def main():
    ...
    for d, f in zip(hk_data,HK_files):
        ...
        print(min(d))
    plt.title('HK array')
    plt.xlabel('signal median - background median')
    plt.legend(loc='best')
    plt.show()
    ...
    if not os.path.exists('signal_minus_background'): os.mkdir('signal_minus_background')
    ...
    for i in range(len(HK_files)):
        ...
        plt.legend(loc='best')
        plt.savefig('signal_minus_background/{}_hist.png'.format(TFs[i]))
    ...
    for i in range(len(HK_files)):
        ...
        plt.legend(loc='best')
        plt.title(TFs[i])
        plt.savefig('signal_minus_background/{}.png'.format(TFs[i]))
