...
def parse_args():
   ...
   # group.add_argument('--max-epochs', type=int, default=25, help='Max epochs')
    ...
    group.add_argument('--TF', type =str, default="blah", help="TF name used to specify optimal stuff")
    ...
    group.add_argument('--weight_error',action='store_true',help='weight BCEloss by class sizes' )
...
def load_data(infile, pad=0, include_reverse_complement=False, sampling="", sampling_factor=5, train=False):
    ...
    mean = np.mean(ys)
    sd_thresh = 2.5*np.std(ys) + mean # fuck fuck fuck i fucked this up
...
def load_data_one_array(infile, pad=0, include_reverse_complement=False, sampling="", sampling_factor=5, train=False):
    ...
    ys = np.array([float(x[1]) for x in lines])
    mean = np.mean(ys)
    sd_thresh = 2.5*np.std(ys) + mean #ok so this shit is not normally distributed
                                    # how about instead of taking 4 stds above I take
   # print(max(ys))
    cutoff = np.where(ys> sd_thresh)
    #cutoff = np.where(ys> 60000)
    # print(cutoff)
    # print(len(cutoff[0]))
    new_ys = np.zeros(len(ys))
    new_ys[cutoff] = 1
...
class Model(nn.Module):
    def __init__(self,motif_detectors, motif_len, fc_nodes):
    #def __init__(self,motif_detectors=64, motif_len=8, fc_nodes=16):
...
def main(args):
    ...
    epochs = args.nepochs
    if args.one_array:
        xdata, ydata, xval, yval, num_low, num_high = load_data_one_array(args.train, opt_params[args.TF][1]-1, args.incl_rev_comp, args.sampling, args.sampling_factor, True)
    ...
    #    if this doesn't work, will see how adding rev comp affects
       # xval, yval = load_data(args.val, args.motif_len-1)
        ...
        xdata, ydata, num_low, num_high = load_data(args.train, opt_params[args.TF][1]-1, args.incl_rev_comp, args.sampling, args.sampling_factor, True)
        xbatched = split_batches(xdata,args.batchsize)
        ybatched = split_batches(ydata,args.batchsize)
        # if this doesn't work, will see how adding rev comp affects
        xval, yval = load_data(args.val, opt_params[args.TF][1]-1)

    model = Model(opt_params[args.TF][0],opt_params[args.TF][1],opt_params[args.TF][2])
   ...
   # BCELoss = nn.BCELoss(weight = weights)
    if args.weight_error:
        weights  = FloatTensor(np.array([(num_low + num_high)/(num_low), (num_low + num_high)/(num_high)]))
        BCELoss = nn.BCELoss(weight = weights)
    else:
        BCELoss = nn.BCELoss()
